...
public class nikonnoticeDAO {
	...
	public ArrayList<nikonnoticeVO> getnikonnoticeList() {
		...
		try {
			...
			String sql = "select * from nikonnotice order by seq desc, seq asc";
	...
	public nikonnoticeVO getnikonnotice(nikonnoticeVO vo) {
		...
		try {
			...
			if(cnt>0) {
				...
				System.out.println("SQL 문 출력 : "+sql);
				stmt = con.prepareStatement(sql);
				System.out.println("해당 글번호 : "+vo.getSeq());
				stmt.setInt(1, vo.getSeq());
				rs = stmt.executeQuery();
	...
	public int addnikonnotice(nikonnoticeVO vo) {
		...
		try {
			con = DBConn.getConnection();
			String sql = "insert into nikonnotice values(nikonnotice_seq.nextval, ?, ?, ?, sysdate,0)";
			stmt = con.prepareStatement(sql);
